 ...
 *
 * O seed é idempotente (usa `upsert`), então pode ser executado várias vezes
 * sem duplicar registros. As imagens vêm de `public/demo` e existem apenas para
 * o projeto já nascer "cheio" — o restaurante substitui tudo em Admin > Mídia.
 */
...
async function seedUser() {
  const email = (process.env.ADMIN_EMAIL ?? "admin@restaurante.com").toLowerCase();
...
async function seedHomeSections() {
  ...
  if (existing > 0) {
    console.log("  · seções da Home já existem — mantidas como estão");
    return;
  }
